    ...
    return `
        <div class="space-y-8 animate-in fade-in duration-500">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="glass-card p-5 rounded-3xl bg-white shadow-sm border border-slate-100">
                    <p class="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-2">Sucessos</p>
                ...
                <div class="glass-card p-5 rounded-3xl bg-white shadow-sm border border-slate-100">
                    ...
                    <p class="text-2xl font-black text-indigo-600">${activeCampaignsCount}</p>
                </div>
                <div class="glass-card p-5 rounded-3xl bg-white shadow-sm border border-slate-100">
            ...
            <section>
                <div class="flex items-center gap-2 mb-4 ml-1">
                    ...
                    <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Disparos em Tempo Real</h4>
                </div>
                <div id="active-monitor-container" class="grid grid-cols-1 md:grid-cols-2 gap-4"></div>
                <div id="overview-empty-state" class="hidden p-8 text-center border-2 border-dashed border-slate-100 rounded-3xl text-[10px] text-slate-300 uppercase font-bold italic">Nenhum envio ativo</div>
            </section>
            ...
            <section>
                <div class="flex items-center gap-2 mb-4 ml-1">
                    ...
                    <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Histórico de Campanhas</h4>
                </div>
                <div id="completed-monitor-container" class="space-y-3">
                    <div class="p-8 text-center text-slate-300 text-[10px] font-bold uppercase italic font-mono">Sincronizando...</div>
                </div>
            </section>
        </div>
    `;
...
Views.startActivePolling = async function() {
    ...
    // Se o container não existir, significa que o usuário mudou de página
    if (!container) {
        if (Views.activePollingTimeout) clearTimeout(Views.activePollingTimeout);
        return;
    }

    // Pula a requisição se a aba estiver escondida
    if (document.hidden) {
                    ...
                    <div class="glass-card p-5 rounded-3xl bg-white border border-slate-100 shadow-sm animate-in fade-in">
                        <div class="flex justify-between items-center mb-4">
                            <div class="flex items-center gap-2">
                                <span class="flex h-1.5 w-1.5 relative"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span><span class="relative inline-flex rounded-full h-1.5 w-1.5 bg-indigo-500"></span></span>
                                <h4 class="text-[11px] font-black text-slate-700 uppercase tracking-tight">${name}</h4>
                            </div>
                            <span class="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg">${percent}%</span>
                        </div>
                        <div class="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                            <div class="h-full bg-indigo-600 transition-all duration-1000" style="width: ${percent}%"></div>
                        </div>
                        <p class="text-[9px] font-bold text-slate-400 uppercase mt-3 tracking-tighter">Processados: ${completed}/${total}</p>
                    </div>`;
            }).join('');
...
Views.initCompletedData = async function() {
    ...
    try {
        ...
        if (completedData.length === 0) {
            container.innerHTML = `<div class="p-8 text-center text-slate-300 text-[10px] font-bold uppercase italic border border-dashed border-slate-100 rounded-3xl">Sem histórico recente</div>`;
            return;
        }
        ...
        container.innerHTML = Object.keys(campaigns).map((name, index) => {
            ...
            return `
                <div class="glass-card bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden animate-in slide-in-from-bottom-2">
                    <button onclick="Views.toggleCampaignDetail('${sectionId}')" class="w-full bg-white hover:bg-slate-50/50 px-6 py-4 flex justify-between items-center transition-all group border-none outline-none">
                        <div class="flex items-center gap-4 text-left">
                            ...
                            <div>
                                <h4 class="text-xs font-black text-slate-800 uppercase tracking-tight">${name}</h4>
                                <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 italic">Template: ${camp.template}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-4">
                            <span class="px-3 py-1 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest">${camp.contacts.length} Envios</span>
                            <i data-lucide="chevron-down" id="icon-${sectionId}" class="w-4 h-4 text-slate-300 transition-transform duration-300"></i>
                        </div>
                    </button>
                        ...
                        <div class="p-4">
                            <div class="max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
                                <table class="w-full text-left text-[10px]">
                                    <thead class="text-[8px] font-black text-slate-300 uppercase tracking-widest sticky top-0 bg-white/80 backdrop-blur-sm">
                                        <tr><th class="px-4 py-2">Destinatário</th><th class="px-4 py-2 text-right">Status</th></tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100">
                                        ${camp.contacts.map(c => `<tr class="text-slate-600"><td class="px-4 py-2"><p class="font-bold">${c.Nome || 'N/A'}</p><p class="text-[8px] font-mono text-slate-400">${c.Telefone || 'N/A'}</p></td><td class="px-4 py-2 text-right text-emerald-500 font-black italic uppercase">Sucesso</td></tr>`).join('')}
                                    </tbody>
                                </table>
                            </div>
                        </div>
        ...
        if (window.lucide) lucide.createIcons();
    } catch (err) { container.innerHTML = `<div class="p-8 text-center text-rose-400 text-[10px] font-bold">Erro ao carregar banco de dados.</div>`; }
};
